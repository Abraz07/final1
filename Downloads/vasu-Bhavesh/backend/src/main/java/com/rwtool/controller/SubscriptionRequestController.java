...
public class SubscriptionRequestController {
    ...
    public ResponseEntity<?> createRequest(@RequestBody SubscriptionRequestDTO requestDTO) {
        try {
            ...
            auditLogService.logSuccess(
                ...
                "Requested access to domain: " + requestDTO.getDomainName()
            );
    ...
    public ResponseEntity<?> approveRequest(
        ...
        try {
            ...
            // Log approval with actual admin info
            auditLogService.logSuccess(
                adminEmail != null ? adminEmail : "admin@rwtool.com",
                adminName != null ? adminName : "Admin",
                "Admin",
                "USER_APPROVED",
                "Approved subscription request for " + request.getUserEmail() + " to domain: " + request.getDomainName()
            );
            return ResponseEntity.ok(request);
        } catch (RuntimeException e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(e.getMessage());
    ...
    public ResponseEntity<?> rejectRequest(
        ...
        try {
            ...
            // Log rejection with actual admin info
            auditLogService.logSuccess(
                adminEmail != null ? adminEmail : "admin@rwtool.com",
                adminName != null ? adminName : "Admin",
                "Admin",
                "USER_REJECTED",
                "Rejected subscription request for " + request.getUserEmail() + " to domain: " + request.getDomainName() + ". Reason: " + reason
            );
            return ResponseEntity.ok(request);
        } catch (RuntimeException e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(e.getMessage());
